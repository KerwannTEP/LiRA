...
function Omega(xi::Float64,x::Float64=1.0, q::Float64=1.0)
    r = radius(xi)
    bulb = (a/c)^3*(1-q*x)/(q*x)*((1-xi)/2)^(-3/2)*(1+(r/c)^2)^(-3/2)
    disk = 1-eps/(q*x)^(2/3)
...
function dOmegadxi(xi::Float64,x::Float64=1.0, q::Float64=1.0)
    r = a/c
    xq = x*q
    num = 3*(-4*r^5*sqrt((2-2*xi)/(1-xi+r^2*(1+xi)))
        + eps*sqrt(1-xi)*(1-xi+r^2*(1+xi))^2/(1/xq)^(1/3)
        - xq*((1-xi)^(5/2)+r^4*sqrt(1-xi)*(1+xi)^2
        - 2*r^2*sqrt(1-xi)*(-1+xi^2)-4*r^5*sqrt((2-2*xi)/(1-xi+r^2*(1+xi)))))
    den = (4*xq*(2-2*xi)^(3/4)*(1-xi+r^2*(1+xi))^2*sqrt(1-1/xq^(2/3)*eps
        +(2*sqrt(2)*r^3*(1-xq)/(1+r^2*(1+xi)/(1-xi))^(3/2))/(xq*(1-xi)^(3/2))))
    return num/den
end
